...
``REPO_ROOT`` / ``MODELS_DIR`` anchor paths when this package lives under ``models/``.
"""
...
# This file lives in ``models/``; repo root holds ``data/``, ``marketshare_data.db``, ``artifacts_75k/``, etc.
...
# Upper bound for product-driven M52-style adjustments in downstream forecast code; mirrored in training metadata.
...
def _adjust_tail_weights_for_empirical_w2(
    ...
    while keeping total tail mass fixed. See ``models.train_marketshare_artifacts.build_artist_w2_retention``.
    """
