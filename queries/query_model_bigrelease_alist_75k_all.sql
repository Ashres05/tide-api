...
-- (per-week 75k A-list AE scrub volumes + big-release flags for 7 labels).
--
-- {MIN_WEEK_END_DATE} is the max WEEK_END_DATE currently in
-- bigrelease_alist_75k_all.csv, injected from Python ('2018-01-01' on cold start).
-- Rolling 18-month first_sale_date freshness for Big_Release_* flags is intentional
-- and is NOT replaced by MIN_WEEK_END_DATE.
...
SELECT
    ...
    SUM(IFF(am.level_2_distributor = 'Interscope-Capitol', wp.equivalent_quantity, 0)) AS INTERSCOPE_CAPITOL_ALBUMS,
    ...
    MAX(IFF(
        am.level_2_distributor = 'Interscope-Capitol'
        AND LOWER(am.display_artist) NOT LIKE '%various%'
        AND wp.first_sale_date >= DATEADD(MONTH, -18, CURRENT_DATE())
        AND wp.first_sale_date BETWEEN DATEADD(DAY, -7, wp.week_end_date) AND wp.week_end_date,
        1, 0
    )) AS BIG_RELEASE_INTERSCOPE_CAPITOL,
