...
#### 6. Call data/refresh_model if running on a cloned repo for the first time (this does not need to be run frequently).
#### 7. Call data/refresh_data if official marketshare data is outdated (can safely be called every Monday morning at 6 A.M. EST).
#### WARNING: these last two calls takes very long, both should not be publicly available for users, should only called in the back-end rarely.
