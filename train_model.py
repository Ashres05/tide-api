...
def _refresh_data_directory() -> None:
    ...
    All three Snowflake queries are independent, so they are submitted
    concurrently via a ThreadPoolExecutor and awaited together.
    """
    logger.info("train_model.py: Updating data directory")
    with get_snowflake_connection() as sf:
        tasks = {
            "Current_Data.csv": lambda: _update_current_data(sf),
            "alist_75k.csv": lambda: _update_a_list_75k(sf),
            "bigreleaseflag_75k.csv": lambda: _update_big_release_flag_75k(sf),
        }
        ...
        for name, future in futures.items():
            ...
            logger.info("train_model.py: Updated %s", name)
